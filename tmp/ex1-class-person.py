# import operator # not used - for loop instead
...
def get_april_birthdays(people):
    ...
    for p in people:
        if p.birthday_month == 'April':
            p.birthday()
            april_birthdays[p.name] = p.age # add key-val pair to new dict
...
def get_most_common_month(people):
    ...
    for p in people:
        if p.birthday_month in months:
            months[p.birthday_month] += 1
            # print(months[p.birthday_month])
    # TODO: Modify the return statement.
    # https://stackoverflow.com/questions/268272/getting-key-with-maximum-value-in-dictionary
    # print(max(months.items(), key = operator.itemgetter(1))[0])
    max_val = -1
    max_key = None
    for month_key, month_value in months.items():
        if month_value > max_val:
            max_val = month_value
            max_key = month_key
