...
Algorithm: Shortest Path (G, s):
    ...
    Output: length of shortest path from s to v for each vertex v of G
        ...
        priority queue Q will contain all vertices of G having D labels
        as key
...
D|v| will always store the length of the best path we found so far.
...
class GraphEdge(object):
    """
    In order to run Dijkstra's Algorithm, we need to add distance
    to each edge.
    """
...
"""
math.inf is useful as an initial value in optimisation problems,
because it works correctly with min,
eg. min(5, math.inf) == 5.

For example, in shortest path algorithms, you can set unknown
distances to math.inf without needing to special case None or
assume an upper bound 9999999.

Similarly, you can use -math.inf as a starting value for maximisation
problems.
"""
