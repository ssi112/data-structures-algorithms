...
def shortest_path(mapx, start, goal):
    ...
    #while not frontier.empty(): # is_empty():
    #    current = frontier.get()
    while not open_set_pq.is_empty():
    ####################################################################
        lowestRemoved = open_set_pq.dequeue()
        kurrent, l_priority = lowestRemoved.element, lowestRemoved.priority
        current = kurrent
        #print("*"*75)
        #print("current={} || kurrent={} | l_priority={}".format(current, kurrent, l_priority))
    ####################################################################
    ...
    # uh oh, frontier is is_empty but goal was not found
    return None
